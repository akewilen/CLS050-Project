...
class Game {
  ...
  List<String> rounds = [];
  Map<int, CountryData?> countryStats = {};  // Using map to store only fetched countries
  List<String> roundStats = [];  // Stores which stat is being compared in each round
  int playerHealth = 3;
...
class GameLogic {
  ...
  static Future<Game?> createGame() async {
    ...
    for (int i = 0; i < game.rounds.length - 1; i++) {
      game.roundStats.add(Game.statNames[random.nextInt(Game.statNames.length)]);
    }
  ...
  static Future<void> _fetchCountriesForCurrentRound(Game game) async {
    ...
    if (!game.countryStats.containsKey(game.currentRoundIndex)) {
      final currentStats = await fetchCountryData(game.rounds[game.currentRoundIndex]);
    ...
    if (!game.countryStats.containsKey(game.currentRoundIndex + 1)) {
      final nextStats = await fetchCountryData(game.rounds[game.currentRoundIndex + 1]);
  ...
  static Future<void> testPrintAllRounds() async {
    ...
    print("Stats: ${game.countryStats[0]?.surfaceArea}, ${game.countryStats[0]?.population}, ${game.countryStats[0]?.co2Emissions}, ${game.countryStats[0]?.forestedArea}, ${game.countryStats[0]?.gdpPerCapita}");
    print("Country 2: ${game.rounds[1]}");
    print("Stats: ${game.countryStats[1]?.surfaceArea}, ${game.countryStats[1]?.population}, ${game.countryStats[1]?.co2Emissions}, ${game.countryStats[1]?.forestedArea}, ${game.countryStats[1]?.gdpPerCapita}");

    while (await nextRound()) {
      print("\nRound ${game.currentRoundIndex + 1}:");
      print("Comparing: ${game.getCurrentStat()}");
      print("Country 1: ${game.rounds[game.currentRoundIndex]}");
      print("Stats: ${game.countryStats[game.currentRoundIndex]?.surfaceArea}, ${game.countryStats[game.currentRoundIndex]?.population}, ${game.countryStats[game.currentRoundIndex]?.co2Emissions}, ${game.countryStats[game.currentRoundIndex]?.forestedArea}, ${game.countryStats[game.currentRoundIndex]?.gdpPerCapita}");
      print("Country 2: ${game.rounds[game.currentRoundIndex + 1]}");
      print("Stats: ${game.countryStats[game.currentRoundIndex + 1]?.surfaceArea}, ${game.countryStats[game.currentRoundIndex + 1]?.population}, ${game.countryStats[game.currentRoundIndex + 1]?.co2Emissions}, ${game.countryStats[game.currentRoundIndex + 1]?.forestedArea}, ${game.countryStats[game.currentRoundIndex + 1]?.gdpPerCapita}");
    }
    print("\nTest complete!");
  }
